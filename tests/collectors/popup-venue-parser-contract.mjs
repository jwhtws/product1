...
import { parsePopupVenuePayload, SourceStructureChangedError } from '../../scripts/parsers/popup-venue-parser.mjs';

export function runPopupVenueParserContract(sourceId) {
  test(`${sourceId}: valid/empty/structure_changed/expired/duplicate/non_food fixture`, async () => {
    const fixture = JSON.parse(await readFile(new URL(`../fixtures/food-popups/${sourceId}.json`, import.meta.url), 'utf8'));
    const parse = items => parsePopupVenuePayload({ ...fixture.source, items }, { today: '2026-08-05' });

    const valid = parse(fixture.valid.items);
    assert.equal(valid.rows.length, 1);
    assert.equal(valid.rows[0].id, `popup-venue:${sourceId}:valid-1`);
