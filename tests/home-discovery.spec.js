...
test('지역과 카테고리 탐색 대신 실제 팝업 지도를 제공한다', async ({ page }) => {
  ...
  await expect(page.locator('#popup-map .leaflet-control-layers-expanded')).toBeVisible({ timeout: 15000 });
  await expect(page.locator('#popup-map .leaflet-control-layers-base label')).toHaveCount(3);
