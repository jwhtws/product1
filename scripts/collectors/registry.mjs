export const COLLECTOR_SCOPES = Object.freeze({
  ...
  'lotte-gwangbok': ['롯데백화점·롯데아울렛·롯데몰'],
  shinsegae: ['신세계백화점'],
  ...
  homeplus: ['홈플러스'],
  malls: ['타임스퀘어 공식 사이트맵'],
  'times-square': ['타임스퀘어 공식 사이트맵'],
  'amore-seongsu': ['팝업 전문 공간 · 아모레성수'],
  'ktng-sangsangmadang': ['팝업 전문 공간 · KT&G 상상마당'],
...
export function selectCollectors(collectors, scope = '') {
  ...
  const byName = new Map(collectors);
  return selected.map(name => byName.has(name) ? [name, byName.get(name)] : null).filter(Boolean);
}
