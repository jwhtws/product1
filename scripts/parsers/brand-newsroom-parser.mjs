...
export function parseBrandNewsroomPayload(payload, { today = new Intl.DateTimeFormat('en-CA', {
  ...
  return {
    ...
    stats,
    sourceHealth: {
      status: rows.length ? 'success_with_items' : payload.verifiedEmptyEvidence ? 'verified_empty' : 'search_incomplete',
      message: rows.length ? `${rows.length}건 파싱` : payload.verifiedEmptyEvidence ? '공식 목록에 현재 행사 없음 확인' : '0건이지만 대체 공식 경로 확인 미완료',
